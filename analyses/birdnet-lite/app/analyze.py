...
def analyseAudioFile(file_path: str, week_of_year: int, lat: float, lon: float, minimum_confidence_threshold: float):
    global WHITE_LIST
    WHITE_LIST = []
    ...
    # Read audio data
    audioData = readAudioData(file_path, 0.0)

    # Process audio data and get detections
    week = max(1, min(week_of_year, 48))
    sensitivity = 1.0
    detections = analyzeAudioData(audioData, lat, lon, week, sensitivity, 0.0, interpreter)

    # Write detections to output file
    min_conf = max(0.01, min(minimum_confidence_threshold, 0.99))   
    ...
    for d in detections:
        for entry in detections[d]:
            if entry[1] >= min_conf and (entry[0] in WHITE_LIST or len(WHITE_LIST) == 0):
                item = d + ';' + entry[0].replace('_', ';') + ';' + str(entry[1])
                items.append(item.split(";"))

    # join consecutive detections
    zipped = []
    for idx, r in enumerate(items): 
        start, end, scientific_name, common_name, confidence = r

        item = [start, end, scientific_name, common_name, confidence]

        if idx > 0:
            p_start, p_end, p_scientific_name, p_common_name, p_confidence = zipped[-1]

            if p_end and scientific_name == p_scientific_name: 
                # Alter the end time of the previous one
                zipped[-1] = [p_start, end, p_scientific_name, p_common_name, p_confidence]
                item = None
    ...
    for z in zipped:
        start, end, scientific_name, common_name, confidence = z
        print(" ", start, end, scientific_name, common_name, confidence)
